...
def dashboard(request):
    ...
    total_amount = loans.aggregate(total=models.Sum("total_loan_amount"))["total"] or 0

    # Removed interest_period_days (field does not exist in model)
    avg_tenure = None  # or set default / placeholder
    ...
    context = {
        ...
        "avg_tenure": avg_tenure,  # keep context variable for template compatibility
        "timeline": [],
    }
...
def loan_entry_step4(request):
    ...
    # Get loan ID from session (same as before)
    loan_id = request.session.get(SESSION_LOAN_ID)
    if not loan_id:
    ...
    # No OTP verification here — JavaScript handles it
    return render(
        request,
...
def loan_payment(request, loan_id):
    """
    Fixed loan payment view — aggregates payments from LoanPayment records
    and computes consistent financial fields for the template.
    """
    loan = get_object_or_404(Loan.objects.select_related('customer'), id=loan_id)
    today = timezone.localdate()

    # 1) Accrue interest up to today (persisted on loan)
    loan.accrue_interest_up_to(today)
    # refresh loan fields from DB in case accrue_interest_up_to updated DB
    loan.refresh_from_db()

    # 2) Aggregate actual payments from LoanPayment (single source of truth)
    ...
    # 3) Values coming from loan model (accrued / pending)
    original_principal = loan.total_loan_amount or Decimal("0.00")
    # principal remaining - prefer model helper
    try:
    ...
    loan_pending_interest = loan.pending_interest or Decimal("0.00")

    # 4) Determine "interest pending" consistently:
    # interest_pending = accrued - paid (but ensure non-negative). If loan.pending_interest field is larger,
    # keep that (DB can have manual pending adjustments).
    interest_pending_calc = (Decimal(total_interest_accrued) - Decimal(interest_paid))
    if interest_pending_calc < 0:
        interest_pending_calc = Decimal("0.00")

    # Use the larger of DB pending_interest and computed pending (to be safe)
    pending_interest = max(interest_pending_calc, Decimal(loan_pending_interest))

    # 5) Daily interest (best effort)
    try:
        daily_interest = quantize_money(loan.daily_interest_amount())
    except Exception:
        daily_interest = quantize_money(Decimal("0.00"))

    # 6) Closing amount = principal remaining + pending interest
    closing_amount = quantize_money(Decimal(principal_remaining) + Decimal(pending_interest))

    # 7) Pending amount (full loan) = closing_amount - total_paid
    pending_amount = quantize_money(Decimal(closing_amount) - Decimal(total_paid))
    if pending_amount < 0:
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            # apply_payment should return breakdown (interest_deducted, principal_deducted)
            ...
            LoanPayment.objects.create(
                ...
                medium=medium,
                notes=notes,
                interest_component=breakdown.get('interest_deducted', Decimal('0.00')),
                principal_component=breakdown.get('principal_deducted', Decimal('0.00')),
            )

            # refresh loan & recompute aggregates
            loan.refresh_from_db()
            payments_agg = LoanPayment.objects.filter(loan=loan).aggregate(
                principal_sum=Sum('principal_component'),
                interest_sum=Sum('interest_component'),
            ...
            messages.success(request, f"Payment ₹{payment_amount} recorded.")
        return redirect('gold_loan:loan_payment', loan_id=loan.id)

    # --------------------------
    # Template context (names match your template)
    # --------------------------
    context = {
        "loan": loan,
...
def home(request):
    context = {
        ...
        "today_collections": LoanPayment.objects.filter(payment_date=timezone.localdate()).count(),
        "recent_loans": Loan.objects.order_by("-created_at")[:7],
        "total_amount": Loan.objects.aggregate(total=models.Sum("total_loan_amount"))["total"] or 0
    }
    ...
    if request.method == "POST":
        ...
        if loan_form.is_valid() and gold_formset.is_valid():
            with transaction.atomic():
                ...
                return redirect("gold_loan:add_loan_step3", customer_id=customer.id)

        # ❌ INVALID – DON'T WIPE THE FORM! JUST RENDER IT BACK
        return render(
            request,
            "gold_loan/loan_entry_step2.html",
            {
                "customer": customer,
                "loan_form": loan_form,        # POST-bound form WITH errors
                "gold_formset": gold_formset,  # POST-bound formset
                "loan": loan,
                "is_existing_customer": True,
            }
        )

    # GET request → show blank/initial form
    else:
        loan_form = LoanForm(instance=loan)
...
def add_loan_step3(request, customer_id):
    ...
    loan_id = request.session.get("EXISTING_CUSTOMER_LOAN_ID")
    if not loan_id:
        messages.error(request, "Start adding loan from Step 2.")
    ...
    if request.method == "POST":
        if document_formset.is_valid():
            ...
            return redirect("gold_loan:add_loan_step4", customer_id=customer_id)
        else:
            messages.error(request, "Please correct the document errors.")

    return render(
        request,
        "gold_loan/loan_entry_step3.html",
        {
...
def add_loan_step4(request, customer_id):
    ...
    # JavaScript handles OTP verification
    if request.method == "POST":
        # OTP verified by JS → save and redirect
